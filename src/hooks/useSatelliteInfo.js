...
const useSatelliteInfo = () => {
  ...
  const getWikiSearch = function(satName, intDesignator) {
    ...
    return Axios.get(url)
      .then(function(response) {
				...
				if (response.data.query.search[0] && response.data.query.search[0].title.includes(checkTitle[0])) {
  ...
  const getWikiSatImg = wikiTitle => {
    ...
    return Axios.get(url)
      .then(response => {
        const imageUrl = response.data.query.pages[Object.keys(response.data.query.pages)[0]].thumbnail ?
          response.data.query.pages[Object.keys(response.data.query.pages)[0]]
            .thumbnail.source : "dummy data"
  ...
  const getWikiSatInfo = (satName, intDesignator) => {
    getWikiSearch(satName, intDesignator).then(title => {
			if (title) {
				Promise.all([getWikiSatDesc(title), getWikiSatImg(title)]).then(values => {
					// console.log(intDesignator)
					setSatInfo(prev =>{return {...prev, [intDesignator]: {
						name: satName,
  ...
  const getWikiAllSatInfo = (categoryIds) => {
        ...
        resolve({
          ...
          long: showPosition.coords.longitude
			...
			for (let id of categoryIds) {
				getSatellites(coords.lat, coords.long, id)
				.then(satellites => {
					console.log('satellites found', satellites)
					if (satellites) {
						satellites.map(satellite => {
							getWikiSatInfo(satellite.satname, satellite.intDesignator)
						})
					}
				});
			}
